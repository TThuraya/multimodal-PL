...
def crop_labels_gan_ct_mri(x):
    return np.logical_and(x > 0, x < 14)
...
def transform_preprocessing_amos(
    ...
    transform_list = [
        ...
        #     margin=[5, 5, 5], select_fn=crop_labels_gan_ct_mri
        # ),
        Spacingd(keys=["image", "label"], pixdim = (1,1,2), mode=['bilinear', 'nearest'])
    ]
